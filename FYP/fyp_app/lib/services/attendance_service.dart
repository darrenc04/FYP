...
class AttendanceService {
      ...
      for (String sessionId in sessionIds) {
        ...
        for (
          ...
          await _firestore.collection('Attendance').doc(docId).set({
            ...
            'startTime': sessionInfoSnap.data()?['start_time'],
            'endTime': sessionInfoSnap.data()?['end_time'],
            'sessionType': sessionDoc.data()?['sessionsType'] ?? 'Class',
          });
  ...
  Future<void> markManualAttendance({
    ...
    // Fetch session details for times and type
    final sessionMasterDoc = await _firestore
        .collection('Sessions')
        .doc(sessionId)
        .get();
    final sessionInfoDoc = await _firestore
        .collection('Sessions')
        .doc(sessionId)
        .collection(dateStr)
        .doc('session_info')
    ...
    final data = {
      ...
      'startTime': sessionInfoDoc.data()?['start_time'],
      'endTime': sessionInfoDoc.data()?['end_time'],
      'sessionType': sessionMasterDoc.data()?['sessionsType'] ?? 'Class',
    };
